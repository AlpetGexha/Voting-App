...
<div
    ...
    @custom-show-report-modal.window="
        isOpen = true
        $nextTick(() => $refs.title.focus())
    "
    x-init="window.livewire.on('reportWasMake', () => {
        isOpen = false
    })"
    ...
    <div class="flex items-end justify-center min-h-screen">
        ...
        <div
            ...
            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <h3 class="text-center text-lg font-medium text-gray-900">{{ __('Report Idea') }}</h3>
                <p class="text-xs text-center leading-5 text-gray-500 px-6 mt-4">
                    {{ __('You can report this only one time.') }}
                </p>

                <form wire:submit.prevent="reported()" action="#" method="POST" class="space-y-4 px-4 py-6">
                    <div>
                        <select wire:model.defer="subject" name="subject_add" id="subject_add"
                            class="w-full bg-gray-100 text-sm rounded-xl border-none px-4 py-2">

                            @php
                                $options = ['Spam', 'Abusive', 'Toxic', 'Racisem', 'Illegal', 'Other'];
                            @endphp
                    ...
                    @enderror
                    <div>
                        <textarea wire:model.defer="reason" name="idea" cols="30" rows="4"
                    ...
                    <div class="flex items-center justify-between space-x-3">
                        ...
                        <button type="submit"
                            ...
                            <span class="ml-1">{{ __('Report') }}</span>
                        </button>
                    </div>
                </form>
            </div>
